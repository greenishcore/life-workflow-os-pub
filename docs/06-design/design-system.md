...
刻度是**从全仓实际用法里量出来的**，按用途命名而不是 xs/sm/md——
这实际是一把 2pt 的梯子，t 恤尺码说不清 6 和 8 该用哪个。

| 令牌 | 值 | 用途 |
|---|---|---|
| `Space.textLine` | 2 | 同一组内两行文字之间 |
| `Space.tight` | 4 | 紧凑元素之间 |
| `Space.inlineTight` | 6 | 行内小元素（徽章、标签）之间 |
| `Space.base` | 8 | **最常用**：小容器内边距、一般间隙 |
| `Space.inline` | 10 | 行内中等元素之间 |
| `Space.block` | 12 | 卡片之间（= `Theme.gap`） |
| `Space.inlineWide` | 14 | 行内宽间隔（并列的信息组） |
| `Space.card` | 16 | 卡片内边距（= `Theme.pad`） |
| `Space.page` | 20 | 页面内边距 |

统计出来的规律：纵向（`VStack(spacing:)`）用 2 / 4，横向（`HStack(spacing:)`）
...
| 令牌 | 值 | 是什么 |
|---|---|---|
| `Layout.tabBarClearance` | 60 | iOS 标签栏高度余量，浮层提示要浮在它上面 |
| `Layout.chartTrailingInset` | 18 | 图表绘图区右侧留白，跟着 `Typo.axis` 走 |

单独放一处，是因为混进 `Space` 会有两个坏处：既被「间距必须在刻度上」的门禁误报，
也会让人以为调这两个数属于调间距——它们各自跟着别的东西走。

### 一段值得记住的返工

这把刻度的第一版是 `2/4/8/12/16/20`，号称「从已经在用的取值里挑的」。
**但挑的时候只统计了恰好落在预设值上的那些，是循环论证**——
结果 6/10/14 这条完整的横向阶梯（共 37 处）被整个判成「违例」，
`ui-spacing-tokens` 报了 63 处待归并，其中 39 处（62%）恰好卡在档位正中间。

那个「62% 都卡在正中间」的分布本身就是刻度错了的证据。
**改刻度前，先去数一遍真实用法，并且要按用途分类数。**
